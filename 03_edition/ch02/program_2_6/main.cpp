...
float inc = 0.25f; // offset for moving the triangle

void init(GLFWwindow* window) {

	renderingProgram = Utils::createShaderProgram("vertShader_2_6.vert", "fragShader_2_6.frag");
...
void display(GLFWwindow* window, double currentTime) {
	...
	x += inc * deltaTime;   // move the triangle along x axis
	if (x > 1.0f) inc = -inc;  // switch to moving the triangle to the left
	if (x < -1.0f) inc = -inc;  // switch to moving the triangle to the right
